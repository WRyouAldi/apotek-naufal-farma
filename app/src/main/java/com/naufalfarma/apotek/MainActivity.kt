...
class MainActivity : Activity() {
    ...
    inner class AppBridge {
        @JavascriptInterface fun scanBarcode() { runOnUiThread {
            val o = com.google.mlkit.vision.codescanner.GmsBarcodeScannerOptions.Builder()
                .setBarcodeFormats(
            ...
            sc.startScan()
                .addOnSuccessListener { b ->
                    val raw = b.rawValue ?: return@addOnSuccessListener
                    webView.evaluateJavascript("window.onNativeBarcodeScanned(${org.json.JSONObject.quote(raw)});", null)
                }
                .addOnFailureListener {
                    Toast.makeText(this@MainActivity, "Scanner tidak dapat dibuka. Pastikan Google Play services tersedia.", Toast.LENGTH_SHORT).show()
                }
        }}

        @JavascriptInterface fun saveReport(filename: String, content: String, mimeType: String) {
            try {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    ...
                    val d = getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS)
                        ?: throw IllegalStateException("Folder Download tidak tersedia")
                    d.mkdirs()
                    val f = File(d, name)
                    FileOutputStream(f).use { it.write(bytes) }
                    toast("Laporan tersimpan: ${f.absolutePath}")
                }
        ...
        @JavascriptInterface fun savePdf(title: String, bodyHtml: String) { runOnUiThread {
            ...
            w.webViewClient = object : WebViewClient() {
                override fun onPageFinished(v: WebView, u: String) {
                    val adapter = v.createPrintDocumentAdapter(title)
                    val attrs = PrintAttributes.Builder()
                        .setMediaSize(PrintAttributes.MediaSize.ISO_A4)
                        .setResolution(PrintAttributes.Resolution("pdf", "pdf", 300, 300))
                        .setMinMargins(PrintAttributes.Margins.NO_MARGINS)
                        .build()
                    val name = title.replace(Regex("[^A-Za-z0-9._-]"), "_") + ".pdf"
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        ...
                        if (uri != null) {
                            contentResolver.openFileDescriptor(uri, "w")?.use { fd -> writePdf(adapter, attrs, fd) }
                            cv.clear(); cv.put(MediaStore.Downloads.IS_PENDING, 0)
                            contentResolver.update(uri, cv, null, null)
                            toast("PDF tersimpan di Download/$name")
                        } else toast("Gagal membuat file PDF")
                    } else {
                        val d = getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS)
                        if (d != null) {
                            d.mkdirs()
                            val f = File(d, name)
                            ParcelFileDescriptor.open(f, ParcelFileDescriptor.MODE_CREATE or ParcelFileDescriptor.MODE_READ_WRITE).use { fd -> writePdf(adapter, attrs, fd) }
                            toast("PDF tersimpan: ${f.absolutePath}")
                        } else toast("Folder Download tidak tersedia")
                    }
                }
            }
        ...
        private fun writePdf(a: PrintDocumentAdapter, attrs: PrintAttributes, fd: ParcelFileDescriptor) {
            a.onLayout(null, attrs, null, object : PrintDocumentAdapter.LayoutResultCallback() {
                override fun onLayoutFinished(info: PrintDocumentInfo, changed: Boolean) {
                    a.onWrite(arrayOf(PageRange.ALL_PAGES), fd, null, object : PrintDocumentAdapter.WriteResultCallback() {
                        override fun onWriteFinished(p: Array<PageRange>) {}
                        override fun onWriteFailed(e: CharSequence?) {}
                    })
                }
            }, null)
        }
