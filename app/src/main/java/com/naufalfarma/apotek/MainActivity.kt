...
class MainActivity : Activity() {
    ...
    inner class AppBridge {
        @JavascriptInterface
        fun seedProducts(json: String): String = productDb.seed(JSONArray(json))

        @JavascriptInterface
        fun searchProducts(query: String, limit: Int): String = productDb.search(query, limit.coerceIn(1, 100))

        @JavascriptInterface
        fun importProducts(json: String, mode: String): String = try {
            productDb.importProducts(JSONArray(json), mode)
        } catch (e: Exception) {
            JSONObject().put("ok", false).put("error", e.message ?: "Import gagal").toString()
        }
                ...
                scanner.startScan()
                    .addOnSuccessListener { barcode ->
                        ...
                        webView.evaluateJavascript(
                            "window.onNativeBarcodeScanned(${JSONObject.quote(raw)});",
                            null
                        )
                    }
            ...
            runOnUiThread {
                ...
                val html = """
                    ...
                    *{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff}
                    ...
                    table{width:100%;border-collapse:collapse}th,td{border:1px solid #888;padding:6px 7px}th{background:#eee}.r{text-align:right}
                    .total{margin:14px 0 0 auto;width:330px}.total th,.total td{font-weight:700}
                    </style></head><body>$bodyHtml</body></html>
                """.trimIndent()
                pdfWebView.webViewClient = object : WebViewClient() {
                    override fun onPageFinished(view: WebView, url: String) {
                        view.postDelayed({ savePdfFromWebView(pdfWebView, title) }, 350)
                    }
                }
                val root = window.decorView as? ViewGroup
                root?.addView(pdfWebView, ViewGroup.LayoutParams(595, ViewGroup.LayoutParams.WRAP_CONTENT))
        ...
        private fun savePdfFromWebView(view: WebView, title: String) {
            try {
                ...
                view.measure(View.MeasureSpec.makeMeasureSpec(pageWidth, View.MeasureSpec.EXACTLY), View.MeasureSpec.makeMeasureSpec(contentHeight, View.MeasureSpec.EXACTLY))
    ...
    class ProductDb(context: android.content.Context) : SQLiteOpenHelper(context, "naufal_products.db", null, 1) {
        ...
        fun seed(rows: JSONArray): String {
            if (count() > 0) return JSONObject().put("ok", true).put("inserted", 0).put("skipped", rows.length()).toString()
            val db = writableDatabase
            db.beginTransaction()
            var inserted = 0
            try {
                for (i in 0 until rows.length()) {
                    if (insertOrUpdate(db, rows.getJSONObject(i), "add_update") == 1) inserted++
                }
        ...
        fun importProducts(rows: JSONArray, mode: String): String {
            val normalizedMode = when (mode.lowercase()) {
                "add", "update", "replace" -> mode.lowercase()
                else -> "add_update"
            }
            val db = writableDatabase
            var added = 0
            ...
            try {
                ...
                for (i in 0 until rows.length()) {
                    when (insertOrUpdate(db, rows.getJSONObject(i), operationMode)) {
                        1 -> added++
                        2 -> updated++
                        else -> skipped++
                    }
        ...
        // 1 = inserted, 2 = updated, 0 = skipped
        ...
        private fun findId(db: SQLiteDatabase, code: String, barcode: String, name: String): Long? {
            if (code.isNotEmpty()) {
                db.rawQuery("SELECT id FROM products WHERE code=? ORDER BY id LIMIT 1", arrayOf(code)).use {
                    if (it.moveToFirst()) return it.getLong(0)
                }
            }
            if (barcode.isNotEmpty()) {
                db.rawQuery("SELECT id FROM products WHERE barcode=? ORDER BY id LIMIT 1", arrayOf(barcode)).use {
                    if (it.moveToFirst()) return it.getLong(0)
                }
            }
            if (name.isNotEmpty()) {
                db.rawQuery("SELECT id FROM products WHERE lower(name)=lower(?) ORDER BY id LIMIT 1", arrayOf(name)).use {
                    if (it.moveToFirst()) return it.getLong(0)
                }
            }
            return null
        ...
        fun search(query: String, limit: Int): String {
            ...
            val cursor = if (q.isEmpty()) {
                db.rawQuery(
                    "SELECT code,barcode,name,jenis,brand,satuan,price,stok,rak FROM products ORDER BY name LIMIT ?",
                    arrayOf(limit.toString())
                )
            } else {
                val like = "%${q.lowercase()}%"
                db.rawQuery(
                    "SELECT code,barcode,name,jenis,brand,satuan,price,stok,rak FROM products WHERE lower(code) LIKE ? OR lower(barcode) LIKE ? OR lower(name) LIKE ? OR lower(jenis) LIKE ? OR lower(brand) LIKE ? ORDER BY CASE WHEN lower(code)=? THEN 0 WHEN lower(barcode)=? THEN 1 WHEN lower(name) LIKE ? THEN 2 ELSE 3 END, name LIMIT ?",
                    arrayOf(like, like, like, like, like, q, q, like, limit.toString())
                )
            }
            val out = JSONArray()
            cursor.use {
                while (it.moveToNext()) {
                    out.put(JSONObject().apply {
                        put("code", it.getString(0) ?: "")
                        put("barcode", it.getString(1) ?: "")
                        put("name", it.getString(2) ?: "")
                        put("jenis", it.getString(3) ?: "")
                        put("brand", it.getString(4) ?: "")
                        put("satuan", it.getString(5) ?: "")
                        put("price", it.getLong(6))
                        put("stok", it.getDouble(7))
                        put("rak", it.getString(8) ?: "")
                    })
                }
            }
            return out.toString()
    ...
    companion object {
        private val PERSISTENT_DB_JS = """
            (function(){
              ...
              function persistentSearch(){renderResults(nativeSearch());}
              function csvParse(text){
                const rows=[];let row=[],cell='',quoted=false;
                for(let i=0;i<text.length;i++){const ch=text[i],nx=text[i+1];if(ch==='"'){if(quoted&&nx==='"'){cell+='"';i++;}else quoted=!quoted;}else if(ch===','&&!quoted){row.push(cell);cell='';}else if((ch==='\n'||ch==='\r')&&!quoted){if(ch==='\r'&&nx==='\n')i++;row.push(cell);if(row.some(v=>v.trim()!==''))rows.push(row);row=[];cell='';}else cell+=ch;}
                row.push(cell);if(row.some(v=>v.trim()!==''))rows.push(row);return rows;
              }
              function norm(s){return String(s||'').toLowerCase().replace(/[^a-z0-9]+/g,'');}
              function toRows(text){
                const rows=csvParse(text);if(rows.length<2)throw new Error('CSV kosong atau hanya berisi header.');
                const h=rows[0].map(norm), idx=(...names)=>{for(const n of names){const i=h.indexOf(norm(n));if(i>=0)return i;}return -1;};
                const ix={code:idx('code','kode','kodeitem','kode barang'),barcode:idx('barcode','bar code'),name:idx('name','nama','namaitem','nama item','nama barang'),jenis:idx('jenis','type','kategori'),brand:idx('brand','merek'),satuan:idx('satuan','unit'),price:idx('price','harga','hargajual','hargajual1','harga jual','harga jual 1'),stok:idx('stok','stock'),rak:idx('rak','lokasi')};
                if(ix.name<0&&ix.code<0&&ix.barcode<0)throw new Error('Kolom nama/kode/barcode tidak ditemukan.');
              ...
              function addImportUi(){
                if(document.getElementById('nativeImportCard'))return;
                const host=document.getElementById('searchArea');if(!host)return;
                const box=document.createElement('div');box.id='nativeImportCard';box.style.cssText='display:flex;gap:7px;align-items:center;margin-top:8px';
                box.innerHTML='<input id="nativeCsvFile" type="file" accept=".csv,text/csv" style="display:none"><select id="nativeCsvMode" style="flex:1;min-width:0;padding:10px;border:1px solid #dfe9e5;border-radius:12px;background:#f8fbfa;font-size:13px"><option value="add_update">Tambah + Update</option><option value="add">Tambah saja</option><option value="update">Update saja</option><option value="replace">Ganti semua</option></select><button type="button" id="nativeCsvBtn" style="white-space:nowrap">📥 Import CSV</button>';
                host.appendChild(box);
                const file=document.getElementById('nativeCsvFile');
                document.getElementById('nativeCsvBtn').onclick=()=>file.click();
                file.onchange=()=>{const f=file.files&&file.files[0];if(!f)return;const mode=document.getElementById('nativeCsvMode').value;const r=new FileReader();r.onload=()=>{try{const rows=toRows(r.result);const res=JSON.parse(Android.importProducts(JSON.stringify(rows),mode));if(!res.ok)throw new Error(res.error||'Import gagal');setStatus('✓ Import selesai: '+res.added+' ditambah, '+res.updated+' diperbarui. Total '+res.total+' produk.');persistentSearch();}catch(e){setStatus('Import gagal: '+e.message,false);}file.value='';};r.readAsText(f,'UTF-8');};
              }
              function enhanceResultMeta(){
                if(window.__naufalRenderWrapped)return;window.__naufalRenderWrapped=true;
                const original=window.renderResults;
                window.renderResults=function(list){
                  original(list);
                  document.querySelectorAll('#results .card').forEach((card,i)=>{const x=window.currentResults&&window.currentResults[i];if(!x)return;const meta=card.querySelector('.meta');const extra=[x.brand,x.satuan,x.stok?('Stok: '+x.stok):''].filter(Boolean).join(' • ');if(extra){const d=document.createElement('div');d.className='meta';d.textContent=extra;if(meta)meta.insertAdjacentElement('afterend',d);else card.querySelector('.name')?.insertAdjacentElement('afterend',d);}});
                };
              }
              function init(){
                try{
                  if(typeof Android==='undefined')return;
                  if(Android.productCount()===0 && Array.isArray(window.ITEMS))for(let i=0;i<ITEMS.length;i+=250)Android.seedProducts(JSON.stringify(ITEMS.slice(i,i+250)));
                  enhanceResultMeta();
                  const q=document.getElementById('q');if(q){const clone=q.cloneNode(true);q.parentNode.replaceChild(clone,q);clone.addEventListener('input',persistentSearch);}
                  addImportUi();persistentSearch();
                  const note=document.querySelector('.small[style*="margin:8px 2px 0"]');if(note)note.innerHTML='Database lokal persistent • '+Android.productCount().toLocaleString('id-ID')+' produk';
                }catch(e){setStatus('Database lokal belum siap: '+e.message,false);}
              }
              setTimeout(init,80);
            })();
        """.trimIndent()
